...
conn = ibm_db.connect("DATABASE=bludb;HOSTNAME=3883e7e4-18f5-4afe-be8c-fa31c41761d2.bs2io90l08kqb1od8lcg.databases.appdomain.cloud;PORT=31498;SECURITY=SSL;SSLSererCertificate=DigiCertGlobalRootCA.crt;UID=kkt67169;PWD=dYoul9V9NLi5dzBo",'','')
...
def editlocation():
    ...
    if result>0:
        return render_template('editlocation.html', locations = locations)
    else:
        msg='No locations found'
        return render_template('editlocation.html', msg=msg)
#Products
@app.route('/products')
def products():
    ...
    if result>0:
        ...
        return render_template('listproduct.html', msg=msg)
...
def locations():
    ...
    if result>0:
        ...
        msg='No locations found'
        return render_template('listlocation.html', msg=msg)
...
def register():
    ...
    if request.method == 'POST' and form.validate():
        ...
        if account:
            ...
            in_sql="INSERT INTO users(name, email, username, password) VALUES(?,?,?,?)"
            pstmt1 = ibm_db.prepare(conn, in_sql)
            ibm_db.bind_param(pstmt1,1,name)
            ibm_db.bind_param(pstmt1,2,email)
            ibm_db.bind_param(pstmt1,3,username)
            ibm_db.bind_param(pstmt1,4,password)
            ibm_db.execute(pstmt1)
...
def editproduct():
    ...
    if result>0:
        return render_template('editproduct.html', products = products)
    else:
        msg='No products found'
...
def deletelocation():
    ...
    if result>0:
        ...
        msg='No products found'
    return render_template('deletelocation.html',msg=msg)
        ...
        if from_location==to_location:
            ...
            if result!=False:
                if(len(result))>0:
                    ...
                    alert_num=q
                    if(alert_num<=0):
                        alert("Please Add some of {} product to {} warehouse!".format(-q,product_id,from_location))
            else:
            ...
            if result!=False:
                if(len(result))>0:
                    ...
                    alert_num=q
                    if(alert_num<=0):
                        alert("Please Add Some of {} product to {} warehouse!It's Running Out of Stock".format(product_id,from_location))
